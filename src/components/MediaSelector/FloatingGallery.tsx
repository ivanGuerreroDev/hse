...
export default class FloatingGallery extends Component<Props, State> {
  ...
  constructor(props: Props) { console.debug('Construct FloatingGallery');
    super(props);
  ...
  shouldComponentUpdate(nextProps: Props, nextState: State) {
    return !_.isEqual(nextState, this.state) || !_.isEqual(nextProps.selectedItemsUri, this.props.selectedItemsUri);
  }

  render() { console.debug(`render FloatingGallery, with ${this.state.mediaUris.length} media files`);
    return (
        ...
        <FlatList
          ...
          renderItem={({item}) =>
            <GalleryFrame
              ...
              onSelect={() => this.props.onSelectMedia?.(item.uri)}
              onUnselect={() => this.props.onUnselectMedia?.(item.uri)}
            />
          }/>
      </View>
    );
