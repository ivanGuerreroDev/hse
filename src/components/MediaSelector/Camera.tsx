...
export default class Camera extends Component<Props, State> {
    ...
    private _onStartRecording() {
        if (this.props.canRecord) {
            ...
            this.rnCameraRef
                ?.recordAsync()
                .then((response) => {
                    console.debug(`Video Captured: ${response.uri}`);
    ...
    private _onCapture() {
        ...
        if (this.state.isRecording) {
            ...
            this.rnCameraRef
                ?.takePictureAsync()
                .then(async (response) => {
                    console.debug(`Photo captured: ${response.uri}`);
                    this.props.onCaptureMedia?.(response.uri);
                    this.setState({
    ...
    constructor(props: Props) {
        console.debug('Construct Camera');
        super(props);

        checkCameraPermission().then((resultCamera) => {
            if (
                resultCamera === RESULTS.GRANTED ||
                resultCamera === RESULTS.LIMITED
            )
                checkMicrophonePermission()
                    ...
                    .catch((error) => console.error(error));
            else this.setState({ cameraPermission: resultCamera });
        });
    }
            ...
            case RESULTS.BLOCKED:
                return (
                    <View style={styles.noCameraLayout}>
                        <Text style={{ textAlign: 'center' }}>
                            <Text>
                                No tiene permisos para acceder a la cámara
                            </Text>
                            {'\n'}
                            <Text
                                style={styles.noCameraLink}
                            ...
                            <Text style={{ fontSize: 10 }}>
                                Si ya concedió permisos a la cámara y sigue
                                viendo esta pantalla, reinicie la aplicación
                            </Text>
                        </Text>
                    </View>
                );
                ...
                return (
                    <View style={styles.CameraLayout}>
                        <View style={{ backgroundColor: 'green', flex: 1 }}>
                            ...
                            >
                                <RecordTimer
                                    ref={(ref) => (this.recordTimerRef = ref)}
                                />
                            </View>
                        </View>
