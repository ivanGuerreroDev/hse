...
const SignComponent = (props: Props) => {
    const { controlBridge, onCancel, onSuccess } = props;
    ...
    useEffect(() => {
        controlBridge?.factory?.documento?.pages?.forEach((page) => {
            page?.controls?.forEach((control) => {
                if (
                    control?.outputValue &&
                    typeof control?.outputValue === 'string'
                ) {
        ...
        <SafeAreaView style={{ backgroundColor: 'fff', flex: 1 }}>
            ...
            <View style={styles.container}>
                <View style={styles.row}>
                    <View>
                        <Text style={styles.puntajeLabel}>
                            Nota:{' '}
                            <Text style={[styles.bold, styles.puntajeText]}>
                                {parseFloat(puntaje?.[0]?.split(':')?.[1])
                                    ?.toFixed(1)
                                    ?.replace('.', ',') || ''}
                            </Text>
                        </Text>
                ...
                <Text style={{ marginBottom: 15 }}>
                    Estoy de acuerdo con la evaluación y firmo conforme:
                </Text>
                <SignatureScreen
                    ref={ref}
                    onOK={handleOK}
