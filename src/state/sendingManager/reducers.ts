    ...
    switch (action.type) {
            ...
            return {
                pendingTasks: [
                    ...state.pendingTasks.filter(
                        (item) => item.documentoId !== pendingTask.documentoId
                    ),
                    pendingTask
                ]
            };
        ...
        case PENDINGMANAGER_ACTIONS.DELETEPENDINGTASK_PENDINGMANAGER: {
            ...
            return {
                pendingTasks: [
                    ...state.pendingTasks.filter(
                        (item) => item.documentoId !== documentoId
                    )
                ]
            };
        }
        ...
        case PENDINGMANAGER_ACTIONS.DELETEPENDINGRESOURCE_PENDINGMANAGER: {
            let { documentoId, resourceUri } = (
                action as DeletePendingResourceAction
            ).payload;
                    ...
                    ...state.pendingTasks
                        ...
                        .map((item) => {
                            return {
                                ...item,
                                pendingResourcesUri: [
                                    ...item.pendingResourcesUri.filter(
    ...
    switch (action.type) {
        ...
        case SENDINGMANAGER_ACTIONS.ADDSENDINGTASK_SENDINGMANAGER: {
            ...
            return {
                ...state,
                sendingTasks: [
                    ...state.sendingTasks.filter(
                        (item) => item.documentoId !== sendingTask.documentoId
                    ),
                    sendingTask
                ]
            };
        ...
        case SENDINGMANAGER_ACTIONS.DELETESENDINGRESOURCE_SENDINGMANAGER: {
            let { documentoId } = (action as DeleteSendingResourceAction)
                .payload;
