...
export default class TextControl extends ControlComponent {
  ...
  render() {
    ...
    let autoCompleteList: JSX.Element[] = ((controlBridge.property('autocomplete') || []) as Array<string>)
      .filter(item => item.toLowerCase().includes((controlBridge.OutputValue as string || '').toLowerCase()))
      .map((item, index) => {
        const regexpResult = RegExp(
          `^(.*)(${(controlBridge.OutputValue as string || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})(.*)$`, 'i'
        ).exec(item) || [];

        return <Text key={index}
          style={{padding: 2, textDecorationLine: 'underline'}}
          onPress={() => {controlBridge.OutputValue = item; this.setState({onEdit: false})}}>
    ...
    let errorMessage: string =
      (controlBridge.property('validate') && controlBridge.OutputValue !== undefined) ?
    ...
    return (
        <View
          style={{paddingBottom: 25-errorHeight}}>
          <Input
            label={controlBridge.property('title')}
            placeholder={controlBridge.property('placeholder')}
            errorMessage={errorMessage}
            onChangeText={(value) => {controlBridge.OutputValue = value, this.setState({onEdit: true})}}
            ...
            errorStyle={{margin: 0, height: errorHeight}}
