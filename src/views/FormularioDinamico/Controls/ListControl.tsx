...
class ListControlModal extends React.Component<ListControlModalProps> {
    ...
    constructor(props: ListControlModalProps) {
        ...
        if (props.selectedData)
            this.state.selectedData = props.multiple
                ? props.selectedData
                : [props.selectedData];
    }
    ...
    render() {
        ...
        const filteredData = data
            .filter(
                (item) =>
                    !this.state.selectedData.find((data) =>
                        _.isEqual(data, item)
                    )
            )
            .filter((item) =>
...
export default class ListControl extends ControlComponent {
    ...
    render() {
        ...
        if (controlBridge.OutputValue)
            selectedValues = controlBridge.property('multiple')
                ? controlBridge.OutputValue
                : [controlBridge.OutputValue];
        else selectedValues = undefined;
        ...
        return (
            ...
            >
                    ...
                    onPress={() =>
                        ...
                        navigation.navigate(
                            ...
                            <ListControlModal
                                ...
                                onSuccess={(output) => {
                                    controlBridge.OutputValue = output;
                                    navigation.goBack();
                                }}
                                selectedData={controlBridge.OutputValue}
                                displayField={controlBridge.property(
                                    'displayfield'
                                )}
                ...
                >
                    {selectedValues
                        ?.map((selectedValue) =>
                            controlBridge.property('displayfield')
                                ? selectedValue[
                                      controlBridge.property('displayfield')
                                  ]
                                : selectedValue
