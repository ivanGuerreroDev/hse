  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <ListItem.Swipeable containerStyle={styles.content}
          leftContent={leftOption} rightContent={rightOption}>
          <ListItem.Content style={styles.content}>
            <Pressable style={{width: '100%'}} onPress={onCardPress}>
              <View style={styles.headerRow}>
                <Text style={styles.documentTitle}>{documento.title}</Text>
                <Text style={styles.documentDate}>{format(modifiedDate, 'dd MMM HH:mm', {locale: es})}</Text>
              </View>
