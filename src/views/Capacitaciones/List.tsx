...
class List extends Component<Props> {
    state = {
        Capacitacion: Object.keys(
            this.props.route.params.participantes[0].reduce(
                (a: any, e: any) => {
            ...
            (abuelo, i) =>
                abuelo && {
                    Participantes: this.props.route.params.participantes[0]
                        .filter(
                            (x: any) => x.IdCapacitacion.toString() === abuelo
                        )
                        .map(
                            (x: any) =>
                                x && {
                    ...
                    <View style={{ height: 450, flex: 1 }}>
                        {
                            <FlatList
                                ...
                                data={this.state.Capacitacion.map(
                                    (x: any) => x
                                )}
                                renderItem={({ item }) => {
                                    return item.Participantes.map((x: any) => (
                                        <CheckBox
                                            checkedColor="#116CB5"
                                            size={20}
                                            title={capitalize(
