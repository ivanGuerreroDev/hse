...
export const createPendingTask = (documento: IDocumento): void => {
    ...
    store.dispatch(
        saveDocumento({
            ...documento,
            resources: [
                ...(documento.resources?.filter(
                    (resource) =>
                        resource.type !== 'object' ||
                        checkResourceIsInUse(resource)
                ) || [])
            ]
        })
    );
    ...
    documento =
        store
            .getState()
            .documentos.documentos.find((item) => item._id === documento._id) ||
        documento;

    const existingPendingTask = store
        .getState()
        .pendingManager.pendingTasks.filter(
            (item) => item.documentoId === documento._id
        );
    ...
    if (existingPendingTask.length === 0) {
        ...
        let pendingTask: IPendingTask = {
            ...
            pendingResourcesUri: createdResources.map(
...
stateMonitor(store, 'sendingManager.isSending', (newValue: boolean) => {
    if (!newValue) startSending();
});
stateMonitor(
    store,
    'sendingManager.sendingTasks',
    (newValue: ISendingTask[], oldValue: ISendingTask[]) => {
        oldValue.forEach((oldSendingTask) => {
            let newSendingTask = newValue.filter(
                (newSendingTask) =>
                    newSendingTask.documentoId === oldSendingTask.documentoId
            );
            if (
                newSendingTask.length === 1 &&
                newSendingTask[0].totalResources === 0
            )
...
const startSendingManager = (): void => {
    if (
        isNetworkAllowed() &&
        !store.getState().sendingManager.isSending &&
        store.getState().pendingManager.pendingTasks.length > 0
    ) {
        ...
        store.getState().pendingManager.pendingTasks.forEach((pendingTask) => {
            store.dispatch(
                addSendingTask({
                    documentoId: pendingTask.documentoId,
                    totalResources: 1
                })
            );
...
const uploadResource = async (
    documentoId: string,
    resizeResourceUri: string,
    oldResourceUri: string
): Promise<void> => {
    try {
        ...
        Upload.addListener('completed', uploadId, () => {
            ...
            const documento: IDocumento = store
                .getState()
                .documentos.documentos.filter(
                    (documento) => documento._id === documentoId
                )[0];

            const resource = (documento.resources || []).find(
                (resource) => resource.url === oldResourceUri
            );
                ...
                store.dispatch(
                    saveDocumento({
                        ...
                        resources: [
                            ...(documento.resources?.filter(
                                (resource) =>
                                    resource.url !== oldResourceUri &&
                                    resource.url !== newResourceUri
                            ) || []),
                            {
                                name: resource.name,
                                url: newResourceUri,
...
const uploadDocumento = async (documentoId: string): Promise<void> => {
    ...
    try {
        ...
        const response = await axios
            .post(`${Config.UrlFormularios}/documentos`, documento)
            .catch((error) => {
                console.log(error);
                throw error;
            });
        ...
        store.dispatch(
            changeStatusDocumento(documentoId, DocumentoStatus.sent)
        );
