...
export class ControlBridge {
    ...
    get OutputValue(): any {
        return this.catchValue(_.cloneDeep(this.control.outputValue));
    }
    ...
    updateDevaultValue() {
        if (this.control.outputMetadata?.defaultValue) {
            if (
                this.control.outputMetadata.updateOnChanges ||
                !this.OutputValue
            ) {
                const newValue = this.catchValue(
                    this.control.outputMetadata.defaultValue
                );
    ...
    validateOutputValue(): string | undefined {
        if (this.control.outputMetadata?.validateSchema) {
            ...
            if (!validate(this.control.outputValue)) {
                return (
                    this.control.outputMetadata.outputValidationErrorMessage ||
                    'unknown'
                );
            }
        }
    ...
    private excecuteValueCode(valueCode: string): any {
        const formatControlReturn = (
            ...
            if (controlBridge === undefined) return undefined;
        ...
        const controls = (
            ...
            filters.forEach((filter) => {
                const controlBridgesFound: Array<ControlBridge> =
                    this.factory.ControlBridgeList.filter((controlBridge) => {
                        return (
                            controlBridge.Path.startsWith(filter.path) &&
                            (controlBridge.Control.type === filter.type ||
                                filter.type === '*')
                        );
                    });
                ...
                controlBridgesFound.forEach((controlBridge) => {
                    if (
                        !controlBridges
                            .map((controlBridge) => controlBridge.Path)
                            .includes(controlBridge.Path)
                    )
                        controlBridges.push(controlBridge);
                });
            });

            return controlBridges.map((controlBridge) =>
                formatControlReturn(controlBridge, properties, output)
            );
        };
        ...
        const resource = (resourceName: string): any => {
            ...
            const localResource: ILocalResource =
                state.resources.resources.filter(
                    (item) =>
        ...
        const lineCode = valueCode?.startsWith('return')
            ? valueCode
            : `return ${valueCode}`;

        return new Function('control', 'controls', 'me', 'resource', lineCode)(
            control,
            controls,
            me,
            resource
